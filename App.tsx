...
import { ArrowRight, Plus, Trash2, LogIn, Upload, X, Edit2, ChevronLeft, Eye, Sliders, Palette, Image as ImageIcon, Download, Clipboard, RefreshCw, Film, Mail, Phone, AlertCircle, Link as LinkIcon } from 'lucide-react';
...
const WorkCard: React.FC<{ work: WorkItem; onClick: () => void }> = ({ work, onClick }) => {
  ...
  return (
    <div 
      onClick={onClick}
      className={`group relative cursor-pointer overflow-hidden rounded-[2rem] bg-[#111] border border-white/5 ${isVertical ? 'aspect-[3/4]' : 'aspect-square'} transition-all duration-700 hover:shadow-2xl hover:border-white/10`}
    >
      {displayMedia ? (
        isVideo ? (
          <video 
...
const VisualItem: React.FC<{ src: string; index: number; isVertical?: boolean; onClick: (src: string) => void }> = ({ src, index, isVertical, onClick }) => {
  const isVideo = isMediaVideo(src);

  return (
    <div 
      onClick={() => onClick(src)}
      className={`overflow-hidden group bg-[#111] rounded-[1.5rem] lg:rounded-[2rem] transition-all duration-500 ${isVertical ? 'aspect-[3/4]' : 'aspect-square'} border border-white/5 relative cursor-pointer hover:shadow-xl hover:border-white/20`}
    >
...
const Lightbox: React.FC<{ src: string; onClose: () => void }> = ({ src, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-start justify-center p-4 sm:p-8 lg:p-10 bg-black/95 backdrop-blur-xl animate-in fade-in duration-300 overflow-y-auto" onClick={onClose}>
      <button 
      ...
      <div className="relative max-w-4xl w-full my-auto flex flex-col items-center justify-center py-10" onClick={e => e.stopPropagation()}>
        {isVideo ? (
...
>
            ...
            {/* Narrative Intro Section - Clean Break */}
            ...
            {/* Content Body - Simple Solid Black Tone */}
...
>
            ...
            <div ref={workSectionRef} className="max-w-7xl mx-auto space-y-12 lg:space-y-20 pt-12 lg:pt-20 pb-20 lg:pb-32 bg-black relative z-10">
                          ...
                          <div className="space-y-8">
                              ...
                              {editingWork.thumbnail && (
                                <div className="p-4 bg-[#111] border border-white/5 rounded-2xl flex items-center justify-center">
                                  {isMediaVideo(editingWork.thumbnail) ? (
                                    ...
                                    <img src={editingWork.thumbnail} className="max-h-32 rounded-xl" onError={(e) => (e.target as HTMLImageElement).src = 'https://via.placeholder.com/150?text=Invalid+URL'} />
                                  )}
                                </div>
                              )}
                            </div>
                            ...
                            <div className="flex gap-2">
                              <input 
                                ...
                                onKeyPress={e => e.key === 'Enter' && addGalleryVisual()}
                              />
                              <button 
                                ...
                                return (
                                  <div key={i} className="relative aspect-square rounded-xl overflow-hidden border border-white/5 bg-[#111]">
                                    {isVid ? (
                                      ...
                                      <img src={v} className="w-full h-full object-cover" onError={(e) => (e.target as HTMLImageElement).src = 'https://via.placeholder.com/150?text=Invalid+URL'} />
                                    )}
                                    <button onClick={() => setEditingWork({...editingWork, visuals: editingWork.visuals.filter((_, idx) => idx !== i)})} className="absolute top-1 right-1 bg-black/80 rounded-full p-1.5 transition-all hover:bg-red-500 shadow-lg"><X size={12}/></button>
                                  </div>
                                );
                              })}
                            </div>
